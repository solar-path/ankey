...
import { useDebugForm } from "@/lib/hooks/useDebugForm";
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
const DOCUMENT_TYPES = [
  ...
  { id: "purchase_request", label: "Purchase Request" },
  { id: "job_description", label: "Job Description" },
  { id: "expense_report", label: "Expense Report" },
  { id: "budget_proposal", label: "Budget Proposal" },
  { id: "contract", label: "Contract" },
];

// Form schemas
const approverFinancialLimitsSchema = z
  .object({
...
const formApproverSchema = z.object({
  ...
  functionalAreas: z.array(z.string()).optional(), // Non-financial criteria
});
...
export function DOAMatrixForm({
  companyId,
  matrix,
  onSuccess,
  onCancel,
}: DOAFormProps) {
  useDebugForm(import.meta.url);
  ...
  const {
    ...
    defaultValues: {
      ...
      effectiveFrom: matrix?.effectiveFrom
        ? new Date(matrix.effectiveFrom).toISOString()
        : undefined,
      effectiveTo: matrix?.effectiveTo
        ? new Date(matrix.effectiveTo).toISOString()
        : undefined,
      ...
      approvalBlocks: matrix?.approvalBlocks || [createDefaultBlock()],
    },
  });
  ...
  // Fetch users and active orgchart
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const usersResponse = await fetch(
          `/api/doa/companies/${companyId}/users`,
          {
            credentials: "include",
          }
        );
        ...
        // Fetch active orgchart
        const orgchartResponse = await fetch(
          `/api/doa/companies/${companyId}/active-orgchart`,
          {
            credentials: "include",
          }
        );
        if (orgchartResponse.ok) {
          ...
          console.log("Fetched orgchart:", orgchartData);
          if (orgchartData.hasActive && orgchartData.orgchart) {
            setHasActiveOrgchart(true);
            setPositions(orgchartData.orgchart.positions || []);
          } else {
            setHasActiveOrgchart(false);
            setPositions([]);
          }
        } else {
          console.error("Failed to fetch orgchart:", orgchartResponse.status);
        }
      } catch (error) {
  ...
  const onSubmit: SubmitHandler<MatrixFormData> = async (data) => {
    setLoading(true);
    try {
      if (isEditing && matrix?.id) {
        const { error } = await client(
          `/api/doa/matrices/${matrix.id}` as any,
          {
            method: "PATCH",
            body: data,
        ...
        if (error) {
          const errorMessage =
            (error.value as any)?.error || "Failed to update approval matrix";
        ...
        const { error } = await (client as any)("/api/doa/matrices", {
          method: "POST",
          body: {
            ...data,
            companyId,
        ...
        if (error) {
          const errorMessage =
            (error.value as any)?.error || "Failed to create approval matrix";
          toast.error(errorMessage);
          return;
        }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Popover open={effectiveFromOpen} onOpenChange={setEffectiveFromOpen}>
            <PopoverTrigger asChild>
              ...
              >
                {effectiveFromDate
                  ? new Date(effectiveFromDate).toLocaleDateString()
                  : "Select date"}
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar
                mode="single"
                selected={
                  effectiveFromDate ? new Date(effectiveFromDate) : undefined
                }
        ...
        <div className="space-y-2">
          ...
          <Popover open={effectiveToOpen} onOpenChange={setEffectiveToOpen}>
            <PopoverTrigger asChild>
              ...
              >
                {effectiveToDate
                  ? new Date(effectiveToDate).toLocaleDateString()
                  : "No end date"}
                <ChevronDownIcon className="size-4" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar
                mode="single"
                selected={
                  effectiveToDate ? new Date(effectiveToDate) : undefined
                }
                captionLayout="dropdown"
                startMonth={
                  effectiveFromDate
                    ? new Date(new Date(effectiveFromDate).getFullYear(), 0)
                    : new Date(new Date().getFullYear(), 0)
                }
                endMonth={new Date(new Date().getFullYear() + 20, 11)}
                disabled={(date) =>
                  effectiveFromDate ? date < new Date(effectiveFromDate) : false
                }
                onSelect={(date) => {
      ...
      <div className="space-y-3">
        ...
        {!hasActiveOrgchart && (
          <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-md">
            ...
            <p className="text-sm text-amber-800">
              No active organization chart found. Only user-based approvers are
              available.
            </p>
          </div>
        )}
        ...
        {errors.approvalBlocks && (
          <p className="text-sm text-destructive">
            {errors.approvalBlocks.message as string}
          </p>
        )}
      </div>
...
function SortableBlockCard({
  ...
  const handleApproverDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = approverFields.findIndex(
        (field) => field.id === active.id
      );
      const newIndex = approverFields.findIndex(
      ...
      setTimeout(() => {
        approverFields.forEach((_, index) => {
          setValue(
            `approvalBlocks.${blockIndex}.approvers.${index}.order`,
            index + 1
          );
        });
      }, 0);
        ...
        <Input
          ...
          onChange={(e) => {
            ...
            if (!financialLimits && value !== undefined) {
              setValue(`${basePath}.financialLimits`, {
                currency: "USD",
                minAmount: value,
              });
            }
          ...
          <PopoverTrigger asChild>
            ...
            >
              {selectedFunctionalAreas.length > 0
                ? `${selectedFunctionalAreas.length} area${
                    selectedFunctionalAreas.length > 1 ? "s" : ""
                  }`
                : "Select criteria"}
              <ChevronsUpDown className="ml-2 size-3 shrink-0 opacity-50" />
            </Button>
                  ...
                  <div key={area.id} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        if (checked) {
                          setValue(`${basePath}.functionalAreas`, [
                            ...current,
                            area.id,
                          ]);
                        } else {
                          setValue(
                            `${basePath}.functionalAreas`,
                            current.filter((faId: string) => faId !== area.id)
                          );
                        }
                      }}
                    />
                    <Label
                      htmlFor={`${basePath}-fa-${area.id}`}
