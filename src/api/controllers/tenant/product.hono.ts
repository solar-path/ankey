...
export const productRoutes = new Hono()
  ...
  .get('/', async c => {
    try {
      ...
      const sortColumn = (products as any)[query.sortBy] || products.createdAt
      const orderByClause = query.sortOrder === 'asc' ? asc(sortColumn) : desc(sortColumn)
      const offset = (query.page - 1) * query.limit

      // Build base query with all conditions at once
      ...
      const [items, totalCount] = await Promise.all([
        finalBaseQuery,
        finalCountQuery.then(result => result[0]?.count || 0),
      ])
