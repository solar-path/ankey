...
describe('Tenant Users API', () => {
  ...
  describe('POST /api/tenant-users/invite', () => {
    it('should invite new user when under limit', async () => {
      ...
      const response = await req.post('/api/tenant-users/invite')
        .send(inviteData)
    ...
    it('should prevent duplicate email invitations', async () => {
      ...
      const response1 = await req.post('/api/tenant-users/invite')
        .send(inviteData)
      expect(response1.status).toBe(201)
      
      // Second invitation should fail
      const response2 = await req.post('/api/tenant-users/invite')
        .send(inviteData)
  ...
  describe('PUT /api/tenant-users/:id', () => {
    it('should update user details', async () => {
      ...
      const response = await req.put(`/api/tenant-users/${session.user.id}`)
        .send(updateData)
    ...
    it('should return 404 for non-existent user', async () => {
      ...
      const response = await req.put('/api/tenant-users/non-existent-id')
