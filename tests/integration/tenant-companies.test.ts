...
import { createAuthenticatedSession, authenticatedRequest, unauthenticatedRequest, useIntegrationTest } from './api-test-helper'
...
describe('Tenant Companies API', () => {
  ...
  describe('POST /api/tenant-companies', () => {
    it('should create new company when under limit', async () => {
      ...
      const response = await req.post('/api/tenant-companies')
        .send(companyData)
    ...
    it('should prevent duplicate company codes', async () => {
      ...
      const response1 = await req.post('/api/tenant-companies')
        .send(companyData)
      expect(response1.status).toBe(201)
      
      // Second company with same code should fail
      const response2 = await req.post('/api/tenant-companies')
        .send({
  ...
  describe('PUT /api/tenant-companies/:id', () => {
    it('should update company details', async () => {
      ...
      const response = await req.put(`/api/tenant-companies/${companies[0].id}`)
        .send(updateData)
    ...
    it('should prevent duplicate codes when updating', async () => {
      ...
      const response = await req.put(`/api/tenant-companies/${companies[1].id}`)
        .send({
  ...
  describe('POST /api/tenant-companies/:id/users', () => {
    it('should add user to company', async () => {
      ...
      const response = await req.post(`/api/tenant-companies/${companies[0].id}/users`)
        .send(userData)
    ...
    it('should prevent adding user twice to same company', async () => {
      ...
      const response1 = await req.post(`/api/tenant-companies/${companies[0].id}/users`)
        .send(userData)
      expect(response1.status).toBe(201)
      
      // Second addition should fail
      const response2 = await req.post(`/api/tenant-companies/${companies[0].id}/users`)
        .send(userData)
      expect(response2.status).toBe(400)
  ...
  describe('DELETE /api/tenant-companies/:id/users/:userId', () => {
    it('should remove user from company', async () => {
      ...
      await req.post(`/api/tenant-companies/${companies[0].id}/users`)
        .send({
          userId: session.user.id,
          role: 'member',
        })
      ...
      // Then remove user
      const response = await req.delete(`/api/tenant-companies/${companies[0].id}/users/${session.user.id}`)
